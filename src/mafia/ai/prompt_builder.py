...
- 생존자: {len(context.get("alive_players", []))}명 ({', '.join(player.name for player in context.get("alive_players", []))})
...
4. 자신의 의견을 기억 정보에 기반하여 논리적으로 설명하세요

응답 규칙:
- conversation: 당신이 발언하고 싶은 내용
"""
    ...
    return (
        _context_prompt(context, game_knowledge) + "\n\n" + conversation_prompt,
...
def generate_prompt_examples():
    ...
    with open("docs/prompt_examples.md", "w", encoding="utf-8") as f:
