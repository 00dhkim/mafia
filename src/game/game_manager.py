...
from ..ai.memory_manager import MemoryType, GamePhase
import random
...
class GameManager:
    ...
    def handle_death(self, player: BasePlayer):
        ...
        try:
            if not player:
                raise ValueError("플레이어가 지정되지 않았습니다.")
            ...
            try:
                ...
                self.logger.error(f"사망 공지 중 오류 발생: {str(e)}")
    ...
    def handle_conversation(self, context: Dict):
        """대화 진행 관리"""
        # 생존한 플레이어들의 대화 순서 관리
        for player in self.alive_players:
    ...
    def run_day_phase(self):
        ...
        3. 투표 진행
        """
        self.update_phase(GamePhase.DAY)
    ...
    def run_night_phase(self):
        ...
        3. 경찰 행동
        """
        self.update_phase(GamePhase.NIGHT)
    ...
    def handle_voting(self):
        """투표 진행 및 결과 처리"""
        try:
            self.update_phase(GamePhase.VOTING)
